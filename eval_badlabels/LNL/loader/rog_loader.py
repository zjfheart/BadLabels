...
def getMNIST(batch_size, TF, data_root='/tmp/public_dataset/pytorch', train_shuffle=True, train=True, val=True, **kwargs):
    num_workers = kwargs.setdefault('num_workers', 1)
    kwargs.pop('input_size', None)
    ...
    if train:
        train_loader = torch.utils.data.DataLoader(
            datasets.MNIST(
                root=data_root, train=True, transform=TF, download=True
            ),
            batch_size=batch_size, shuffle=train_shuffle, **kwargs)
        ds.append(train_loader)
    if val:
        test_loader = torch.utils.data.DataLoader(
            datasets.MNIST(
                root=data_root, train=False, transform=TF, download=True
            ),
            batch_size=batch_size, shuffle=False, **kwargs)
        ds.append(test_loader)
    ds = ds[0] if len(ds) == 1 else ds
...
def getTargetDataSet(data_type, batch_size, input_TF, dataroot, train_shuffle = True):
    if data_type == 'cifar10':
        ...
        train_loader, test_loader \
        = getMNIST(batch_size=batch_size, TF=input_TF, data_root=dataroot, train_shuffle=train_shuffle, num_workers=1)
